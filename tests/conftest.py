    ...
    for i in range(12):
        cases.append(
            MissingPersonCase(
                ...
                date_of_birth=datetime(1990 + i, (i % 12) + 1, (i % 28) + 1),
                gender="Male" if i % 2 == 0 else "Female",
                last_seen_date=datetime.now() - timedelta(days=i + 1),
                last_seen_location=sample_location,
                status=CaseStatus.ACTIVE,
                circumstances=f"Test circumstances {i + 1}",
                reporter_name=f"Reporter{i + 1}",
                reporter_phone=f"+39 333 {1000000 + i}",
                relationship="Family" if i % 2 == 0 else "Friend",
