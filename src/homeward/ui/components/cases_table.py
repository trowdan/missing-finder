...
def create_case_row(case: MissingPersonCase, on_click: Optional[Callable] = None, is_last: bool = False):
    ...
    with ui.element('div').classes(row_classes).on('click', on_click if on_click else lambda: None):
        ...
        ui.label(f'{case.last_seen_location.address}, {case.last_seen_location.city}').classes('text-gray-300 text-sm truncate')
        ui.label(case.last_seen_date.strftime('%d/%m/%Y %H:%M')).classes('text-gray-400 text-sm')
        ...
        with ui.element('div').classes('flex justify-center'):
            ...
            ui.label(case.priority.value).classes(f'px-2 py-1 rounded-full text-white text-xs {priority_color}')
        ...
        with ui.element('div').classes('flex justify-center'):
            ...
            ui.label(case.status.value).classes(f'px-2 py-1 rounded-full text-white text-xs {status_color}')
...
def create_cases_table(cases: list[MissingPersonCase], on_case_click: Optional[Callable] = None, on_view_all_click: Optional[Callable] = None):
    ...
    with ui.element('div').classes('w-full bg-gray-800/30 rounded-lg border border-gray-700/50 overflow-hidden'):
        ...
        with ui.element('div').classes('grid grid-cols-8 gap-4 px-6 py-4 bg-gray-800/70 border-b border-gray-700/50'):
            ...
            ui.label('Priority').classes('text-gray-300 font-medium text-sm text-center')
