            ...
            with ui.column().classes("w-full max-w-4xl space-y-6"):
                ...
                with ui.card().classes(
                    ...
                    with ui.grid(columns="1fr 1fr").classes(
                        ...
                        with ui.column():
                            ...
                            form_data["surname"].on(
                                "blur",
                                lambda: validate_name_field(
                                    form_data["surname"], "Last name"
                                ),
                            )
                    ...
                    with ui.row().classes("items-center mb-4"):
                        ...
                        ui.label("Physical Description").classes(
                    ...
                    form_data["distinguishing_marks"] = (
                        ui.textarea(
                ...
                with ui.card().classes(
                    ...
                    with ui.row().classes("items-center mb-6"):
                        ...
                        ui.label("Circumstances & Details").classes(
                            "text-2xl font-light text-white"
                        )

                    with ui.row().classes("items-center mb-4"):
                        ui.icon("dry_cleaning", size="1.2rem").classes(
                            "text-gray-400 mr-2"
                        )
                    ...
                    form_data["clothing_description"] = (
                        ui.textarea(
                    ...
                    with ui.row().classes("items-center mb-4"):
                        ui.icon("help_outline", size="1.2rem").classes(
                            "text-gray-400 mr-2"
                        )
                        ui.label("Circumstances of Disappearance").classes(
                            "text-lg font-light text-gray-200"
                        )
                ...
                with ui.card().classes(
                    ...
                    with ui.row().classes("items-center mb-6"):
                        ui.icon("medical_services", size="1.5rem").classes(
                ...
                with ui.card().classes(
                    ...
                    with ui.grid(columns="1fr 1fr").classes(
                        ...
                        with ui.column():
                            ...
                            form_data["reporter_name"].on(
                                "blur",
                                lambda: validate_name_field(
                                    form_data["reporter_name"], "Reporter name"
                                ),
                            )
                        with ui.column():
                            ...
                            form_data["reporter_phone"].on(
                                "blur",
                                lambda: validate_phone_field(
                                    form_data["reporter_phone"]
                                ),
                            )
                        ...
                        with ui.column():
                            ...
                            form_data["reporter_email"].on(
                                "blur",
                                lambda: validate_email_field(
                                    form_data["reporter_email"]
                                ),
                            )
                        with ui.column():
                ...
                # Action Buttons
                with ui.column().classes(
                    "w-full items-center gap-6 sm:flex-row sm:justify-center mt-8"
                ):
...
def handle_submit(form_data: dict, on_submit: Callable[[dict], None], is_loading: dict, submit_button, cancel_button, edit_mode: bool = False):
    ...
    try:
        ...
        for field in required_fields:
            if (
                field in form_data
                and form_data[field].value
                and str(form_data[field].value).strip()
            ):
                continue
        ...
        if len(address) < 5:
            ui.notify(
                "Please provide a more detailed address (minimum 5 characters)",
                type="negative",
            )
            reset_loading_state()
            return
        ...
        if len(circumstances) < 10:
            ui.notify(
                "Please provide more detailed circumstances (minimum 10 characters)",
                type="negative",
            )
            reset_loading_state()
            return
        if len(circumstances) > 2000:
            ui.notify(
                "Circumstances description is too long (maximum 2000 characters)",
                type="negative",
            )
            reset_loading_state()
            return
        ...
        if not re.match(name_pattern, reporter_name):
            ui.notify(
                "Reporter name can only contain letters, spaces, hyphens, and apostrophes",
                type="negative",
            )
            reset_loading_state()
            return
        ...
        if len(relationship) < 2:
            ui.notify(
                "Please specify the relationship (minimum 2 characters)", type="negative"
            )
            reset_loading_state()
            return
        ...
        if (
            form_data.get("clothing_description")
            and form_data["clothing_description"].value
        ):
            clothing = form_data["clothing_description"].value.strip()
            if len(clothing) > 1000:
                ui.notify(
                    "Clothing description is too long (maximum 1000 characters)",
                    type="negative",
                )
                reset_loading_state()
                return
        ...
        if form_data.get("additional_info") and form_data["additional_info"].value:
            ...
            if len(additional) > 1000:
                ui.notify(
                    "Additional information is too long (maximum 1000 characters)",
                    type="negative",
                )
                reset_loading_state()
                return
        ...
        # Call the submission handler with async handling
        def handle_async_submission():
            try:
                on_submit(submission_data, reset_loading_state)
