...
def create_new_sighting_page(data_service: DataService, config: AppConfig, on_back_to_dashboard: callable):
    ...
    with ui.column().classes('w-full bg-gradient-to-br from-gray-950 via-gray-900 to-gray-950 min-h-screen'):
        ...
        with ui.column().classes('max-w-7xl mx-auto p-8 w-full'):
            ...
            with ui.column().classes('items-center text-center mb-12'):
                ...
                ui.label('New Sighting Report').classes('text-2xl font-light text-gray-300 tracking-wide')
            ...
            with ui.column().classes('w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8'):
                ...
                with ui.column().classes('items-center justify-center text-center gap-4 w-full mb-8'):
                    ...
                    ui.label('Sighting Report').classes('text-3xl sm:text-4xl lg:text-5xl font-extralight text-white tracking-wide')
                    ui.label('Help us locate missing persons by reporting sightings with detailed information').classes('text-gray-400 text-base sm:text-lg font-light px-4 text-center')
                    ...
                    with ui.column().classes('w-full max-w-4xl space-y-6'):
                        ...
                        with ui.card().classes('w-full p-6 bg-gray-900/50 backdrop-blur-sm border border-gray-800/50 shadow-none rounded-xl hover:bg-gray-800/30 transition-all duration-300'):
                            ...
                            with ui.grid(columns='1fr 1fr').classes('w-full gap-4 grid-cols-1 sm:grid-cols-2'):
                                with ui.column():
                                    form_data['reporter_name'] = ui.input('Full Name*').classes('w-full bg-gray-700/50 text-white border-gray-500 rounded-lg').props('outlined dense')
                                with ui.column():
                                    form_data['reporter_email'] = ui.input('Email Address').classes('w-full bg-gray-700/50 text-white border-gray-500 rounded-lg').props('outlined dense')

                                with ui.column():
                                    form_data['reporter_phone'] = ui.input('Phone Number').classes('w-full bg-gray-700/50 text-white border-gray-500 rounded-lg').props('outlined dense')
                                with ui.column():
                                    form_data['relationship'] = ui.input('Relationship to Missing Person').classes('w-full bg-gray-700/50 text-white border-gray-500 rounded-lg').props('outlined dense')
                        ...
                        with ui.card().classes('w-full p-6 bg-gray-900/50 backdrop-blur-sm border border-gray-800/50 shadow-none rounded-xl hover:bg-gray-800/30 transition-all duration-300'):
                            ...
                            with ui.grid(columns='1fr 1fr').classes('w-full gap-4 mb-6 grid-cols-1 sm:grid-cols-2'):
                                with ui.column():
                                    with ui.input('Date of Sighting*').classes('w-full bg-gray-700/50 text-white border-gray-500 rounded-lg').props('outlined dense readonly') as sighting_date_input:
                                        with ui.menu().props('no-parent-event') as sighting_date_menu:
                                            sighting_date_picker = ui.date().classes('bg-gray-700')
                                        sighting_date_input.props('append-icon=event')
                                        sighting_date_input.on('click', sighting_date_menu.open)
                                        sighting_date_picker.on('update:model-value', lambda e: (
                                            sighting_date_input.set_value(e.args[0] if e.args else ''),
                                            sighting_date_menu.close()
                                        ))
                                        form_data['sighting_date'] = sighting_date_input
                                with ui.column():
                                    with ui.input('Time of Sighting').classes('w-full bg-gray-700/50 text-white border-gray-500 rounded-lg').props('outlined dense readonly') as sighting_time_input:
                                        with ui.menu().props('no-parent-event') as sighting_time_menu:
                                            sighting_time_picker = ui.time().classes('bg-gray-700')
                                        sighting_time_input.props('append-icon=access_time')
                                        sighting_time_input.on('click', sighting_time_menu.open)
                                        sighting_time_picker.on('update:model-value', lambda e: (
                            ...
                            form_data['sighting_address'] = ui.input('Address/Location*', placeholder='Street address where sighting occurred').classes('w-full bg-gray-700/50 text-white border-gray-500 rounded-lg mb-4').props('outlined dense')

                            with ui.grid(columns='1fr 1fr 1fr').classes('w-full gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3'):
                                with ui.column():
                                    form_data['sighting_city'] = ui.input('City*', placeholder='City name').classes('w-full bg-gray-700/50 text-white border-gray-500 rounded-lg').props('outlined dense')
                                with ui.column():
                                    form_data['sighting_postal'] = ui.input('Postal Code', placeholder='Postal/ZIP code').classes('w-full bg-gray-700/50 text-white border-gray-500 rounded-lg').props('outlined dense')
                                with ui.column():
                                    form_data['sighting_landmarks'] = ui.input('Landmarks/Details', placeholder='Nearby landmarks').classes('w-full bg-gray-700/50 text-white border-gray-500 rounded-lg').props('outlined dense')
                        ...
                        with ui.card().classes('w-full p-6 bg-gray-900/50 backdrop-blur-sm border border-gray-800/50 shadow-none rounded-xl hover:bg-gray-800/30 transition-all duration-300'):
                            ...
                            with ui.grid(columns='1fr 1fr').classes('w-full gap-4 grid-cols-1 sm:grid-cols-2'):
                                with ui.column():
                                    form_data['individual_age'] = ui.number('Estimated Age', min=0, max=150).classes('w-full bg-gray-700/50 text-white border-gray-500 rounded-lg').props('outlined dense')
                                with ui.column():
                                    form_data['individual_gender'] = ui.select(
                                        ['Male', 'Female', 'Non-binary', 'Unknown'],
                                        label='Gender'
                                    ).classes('w-full bg-gray-700/50 text-white border-gray-500 rounded-lg').props('outlined dense')

                                with ui.column():
                                    form_data['individual_height'] = ui.input('Height', placeholder='e.g., 5\'6", 170cm').classes('w-full bg-gray-700/50 text-white border-gray-500 rounded-lg').props('outlined dense')
                                with ui.column():
                                    form_data['individual_build'] = ui.input('Build', placeholder='e.g., Slim, Average, Heavy').classes('w-full bg-gray-700/50 text-white border-gray-500 rounded-lg').props('outlined dense')

                                with ui.column():
                                    form_data['individual_hair'] = ui.input('Hair Color', placeholder='Hair color and style').classes('w-full bg-gray-700/50 text-white border-gray-500 rounded-lg').props('outlined dense')
                                with ui.column():
                                    form_data['individual_features'] = ui.input('Distinctive Features', placeholder='Scars, tattoos, glasses, etc.').classes('w-full bg-gray-700/50 text-white border-gray-500 rounded-lg').props('outlined dense')
                        ...
                        with ui.card().classes('w-full p-6 bg-gray-900/50 backdrop-blur-sm border border-gray-800/50 shadow-none rounded-xl hover:bg-gray-800/30 transition-all duration-300'):
                            ...
                            with ui.grid(columns='1fr 1fr').classes('w-full gap-4 grid-cols-1 sm:grid-cols-2'):
                                with ui.column():
                                    form_data['clothing_upper'] = ui.input('Upper Body Clothing', placeholder='Shirt, jacket, etc.').classes('w-full bg-gray-700/50 text-white border-gray-500 rounded-lg').props('outlined dense')
                                with ui.column():
                                    form_data['clothing_lower'] = ui.input('Lower Body Clothing', placeholder='Pants, skirt, etc.').classes('w-full bg-gray-700/50 text-white border-gray-500 rounded-lg').props('outlined dense')

                                with ui.column():
                                    form_data['clothing_shoes'] = ui.input('Footwear', placeholder='Shoes, boots, etc.').classes('w-full bg-gray-700/50 text-white border-gray-500 rounded-lg').props('outlined dense')
                                with ui.column():
                                    form_data['clothing_accessories'] = ui.input('Accessories', placeholder='Bag, jewelry, hat, etc.').classes('w-full bg-gray-700/50 text-white border-gray-500 rounded-lg').props('outlined dense')
                        ...
                        with ui.card().classes('w-full p-6 bg-gray-900/50 backdrop-blur-sm border border-gray-800/50 shadow-none rounded-xl hover:bg-gray-800/30 transition-all duration-300'):
                            ...
                            form_data['behavior'] = ui.textarea(
                                'Behavior Observed', placeholder='Describe the person\'s behavior, demeanor, or actions'
                            ).classes('w-full min-h-20 bg-gray-700/50 text-white border-gray-500 rounded-lg mb-4').props('outlined')

                            form_data['condition'] = ui.textarea(
                                'Apparent Condition', placeholder='Physical or mental state, any signs of distress'
                            ).classes('w-full min-h-20 bg-gray-700/50 text-white border-gray-500 rounded-lg mb-4').props('outlined')

                            form_data['direction'] = ui.input('Direction of Travel', placeholder='Which way was the person heading?').classes('w-full bg-gray-700/50 text-white border-gray-500 rounded-lg').props('outlined dense')
                        ...
                        with ui.card().classes('w-full p-6 bg-gray-900/50 backdrop-blur-sm border border-gray-800/50 shadow-none rounded-xl hover:bg-gray-800/30 transition-all duration-300'):
                            ...
                            form_data['additional_details'] = ui.textarea(
                                'Additional Details', placeholder='Any other relevant information about the sighting'
                            ).classes('w-full min-h-32 bg-gray-700/50 text-white border-gray-500 rounded-lg mb-6').props('outlined')

                            form_data['confidence'] = ui.select([
                                'Very High - I\'m certain it was them',
                                'High - Very likely it was them',
                                'Medium - Possibly them',
                                'Low - Uncertain but worth reporting'
                            ], label='Confidence Level').classes('w-full bg-gray-700/50 text-white border-gray-500 rounded-lg').props('outlined dense')

                        # AI-Powered Semantic Matching Section
                        with ui.card().classes('w-full p-6 bg-gray-900/50 backdrop-blur-sm border border-gray-800/50 shadow-none rounded-xl hover:bg-gray-800/30 transition-all duration-300 relative'):
                            # AI badge in corner
                            ...
                            with ui.row().classes('items-center mb-6'):
                                ...
                                ui.label('Find Similar Cases').classes('text-xl sm:text-2xl font-light text-white')

                            ui.label('Before submitting, search for potentially matching missing person cases using AI-powered semantic analysis.').classes('text-gray-400 text-sm mb-6')
                            ...
                            # Results container
                            with ui.column().classes('w-full bg-gray-800/30 rounded-lg p-6 border border-gray-700/50 relative') as semantic_results_container:
                                with ui.row().classes('items-center mb-4'):
                            ...
                            search_button.on('click', lambda: handle_semantic_search(form_data, data_service, semantic_results_container))
                        ...
                        with ui.row().classes('w-full justify-center gap-6 mt-12'):
                            ...
                            ui.button(
                                ...
                                on_click=lambda: handle_form_submission(form_data, data_service)
...
def handle_form_submission(form_data: dict, data_service: DataService):
    ...
    try:
        ...
        required_fields = ['reporter_name', 'sighting_date', 'sighting_address', 'sighting_city']
        for field in required_fields:
            if not sighting_data.get(field):
                ui.notify(f'Please fill in the required field: {field.replace("_", " ").title()}', type='negative')
                return
...
def handle_semantic_search(form_data: dict, data_service: DataService, results_container):
    ...
    try:
        ...
        if not sighting_data.get('individual_age') and not sighting_data.get('individual_gender') and not sighting_data.get('clothing_upper'):
            ui.notify('Please fill in some individual description fields before searching', type='warning')
            return
            ...
            with ui.element('div').classes('w-full bg-gray-800/30 rounded-lg border border-gray-700/50 overflow-hidden'):
                ...
                for i, match in enumerate(matches):
                    ...
                    with ui.element('div').classes(row_classes):
                        ...
                        # View Case button
                        with ui.element('div').classes('flex justify-center'):
                            ui.button('View', on_click=lambda c=case: handle_view_case(c.id)).classes('bg-transparent text-blue-300 px-3 py-1 rounded border border-blue-500/60 hover:bg-blue-200 hover:text-blue-900 hover:border-blue-200 transition-all duration-300 font-light text-xs tracking-wide')

                        # Link Sighting button
                        with ui.element('div').classes('flex justify-center'):
                            ui.button('Link', on_click=lambda c=case: handle_link_to_case(c.id)).classes('bg-transparent text-green-300 px-3 py-1 rounded border border-green-500/60 hover:bg-green-200 hover:text-green-900 hover:border-green-200 transition-all duration-300 font-light text-xs tracking-wide')
